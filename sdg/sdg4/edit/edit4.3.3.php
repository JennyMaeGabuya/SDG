...
if(isset($_GET['ppa_id'])) //the ppa_id came from 4.3.1.php which was declared as variable to edit record
    {
     ...
     $query = "SELECT * FROM `tbl433_vocational` WHERE `total_number` = $ppa_id";
 ...
 if (isset ($_POST['update'])){
    ...
    $fund = strtoupper($_POST['fund']);
      ...
      // SQL query to update the data in user table where the total_number = $ppa_id 
        $query ="UPDATE `tbl433_vocational` SET   `title` = '{$title}', `description` = '{$desc}', `cost` = '{$cost}',`fund` = '{$fund}' WHERE `total_number`= $ppa_id";
