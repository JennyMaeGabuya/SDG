...
if(isset($_GET['ppa_id'])) //the ppa_id came from 15.4.3.php which was declared as variable to edit record
    {
      $ppa_id = $_GET['ppa_id']; 
    }
    
      // SQL query to select all the data from the table where researchid = $researchid
     $query = "SELECT * FROM `tbl15_42_food` WHERE `ID` = $ppa_id";
 ...
 if (isset ($_POST['update'])){
    ...
    $fund = $_POST['fund'];
      ...
      // SQL query to update the data in user table where the total_number = $ppa_id 
        $query ="UPDATE `tbl15_42_food` SET   `total_policies` = '{$policies}',  `title` = '{$title}', `description` = '{$desc}', `cost` = '{$cost}',`fund` = '{$fund}'
         WHERE `ID`= $ppa_id";
...
<form method="POST">
  ...
  <div class="form-group">
      <label for="cost">Total number of policies ensuring that food on campus is sustainably farmed</label>
      <input type="number" class="form-control" id="" name="policies"value="<?php echo $policies ?>" required>
    </div>     
...
<button type="reset" class="btn btn-danger mt-6 mb-3" name="cancel" onclick="window.location.href='../15.4.2.php';">
