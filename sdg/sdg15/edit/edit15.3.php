...
if(isset($_GET['ppa_id'])) //the ppa_id came from 15.3.php which was declared as variable to edit record
    {
      $ppa_id = $_GET['ppa_id']; 
    }
    
      // SQL query to select all the data from the table where researchid = $researchid
     $query = "SELECT * FROM `tbl15_3_endangered` WHERE `ID` = $ppa_id";
...
<form method="POST">
  ...
  <div class="form-group"><i class="fa fa-dollar"></i>
      ...
      <input type="number" class="form-control" id="" name="total" required>
    </div>
...
<button type="reset" class="btn btn-danger mt-6 mb-3" name="cancel" onclick="window.location.href='../15.3.php';">
