...
if(isset($_GET['access_id'])) //the access_id came from 4.3.5.php which was declared as variable to edit record
    {
      $access_id = $_GET['access_id']; 
    }
    
      // SQL query to select all the data from the table 
     $query = "SELECT * FROM `tbl435_access` WHERE `ID` = $access_id";
...
<form method="POST">
  ...
  <div class="form-group"><i class="fa fa-dollar"></i>
      ...
      <input type="number" class="form-control" id="" name="totalstud" value="<?php echo $totalstud ?>" >
    </div>
    <div class="form-group"><i class="fa fa-dollar"></i>
      <label for="cost">Percentage of the student population aged 50 years old and above</label>
      <input type="number" class="form-control" id="" name="percent50above" placeholder="%" value="<?php echo $percent ?>" >
    </div>
    <div class="form-group"><i class="fa fa-dollar"></i>
      <label for="cost">Total number of learning access policy</label>
      <input type="number" class="form-control" id="" name="policy"value="<?php echo $policy ?>" >
    </div>
    ...
    <div class="form-group"><i class="fa fa-bar-chart"></i>
      ...
      <textarea class="form-control" id="title" name="title" rows="3" value=""><?php echo $title ?></textarea>
    </div>
    <div class="form-group"><i class="fa fa-area-chart"></i>
      <label for="desc">Short description of the PPA</label>
      <textarea class="form-control" id="desc" name="desc" rows="5" value="" ><?php echo $desc ?></textarea>
    </div>
    <div class="form-group"><i class="	fa fa-book"></i>
      <label for="cost">Total cost</label>
      <input type="number" class="form-control" id="" name="cost" value="<?php echo $cost ?>" >
    </div>
    <div class="form-group"><i class="fa fa-drivers-license"></i>
      <label for="fund">Fund source</label>
      <input type="text" class="form-control" id="" name="fund" value="<?php echo $fund ?>" >
    </div>
...
<button type="reset" class="btn btn-danger mt-6 mb-3" name="cancel" onclick="window.location.href='../4.3.5.php';">
