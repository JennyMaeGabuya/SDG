...
if(isset($_GET['ppa_id'])) //the ppa_id came from 15.4.3.php which was declared as variable to edit record
    {
     ...
     $query = "SELECT * FROM `tbl15_43_ecosystem` WHERE `ID` = $ppa_id";
 ...
 if (isset ($_POST['update'])){
    ...
    $fund = $_POST['fund'];
      ...
      // SQL query to update the data in user table where the total_number = $ppa_id 
        $query ="UPDATE `tbl15_43_ecosystem` SET   `title` = '{$title}', `description` = '{$desc}', `cost` = '{$cost}',`fund` = '{$fund}'
         WHERE `ID`= $ppa_id";
...
<button type="reset" class="btn btn-danger mt-6 mb-3" name="cancel" onclick="window.location.href='../15.4.3.php';">
